...
async def on_message(message):
    ...
    # dink @user
    if message.content.startswith('d!dink'):
        given_name = message.content[7:]
        try:
            user = discord.utils.get(client.get_all_members(), name=given_name).id
    ...
    # doink @user
    if message.content.startswith('d!doink'):
        given_name = message.content[8:]
        try:
            user = discord.utils.get(client.get_all_members(), name=given_name).id
        except AttributeError:
            return
    ...
    # special dink @user
    if message.content.startswith('d!special'):
        given_name = message.content[10:]
        try:
            user = discord.utils.get(client.get_all_members(), name=given_name).id
        except AttributeError:
            return
    ...
    # random dink
    if message.content.startswith('d!random'):
        msg = find_dink()
